...
def evaluate_entry(
    ...
    *,
    ...
    prior_report = (
        prior_request.get("report", {})
        if isinstance(prior_request.get("report"), dict)
        else {}
    )
    ...
    def run_in_workspace(workspace: Path) -> dict[str, Any]:
        ...
        if not library.is_dir():
            library = paired_answer_web_run.prepare_workspace(
                source_library, workspace, entry_id
            )
        paired_answer_web_run.configure_server(library, workspace)
        entry = library / "entries" / entry_id
        handler = object.__new__(server.Handler)
        return handler.run_w3_shadow_analysis(entry, {
    ...
    shadow = (
        report.get("w3r_shadow", {})
        if isinstance(report.get("w3r_shadow"), dict)
        else {}
    )
    render = (
        shadow.get("render_result", {})
        if isinstance(shadow.get("render_result"), dict)
        else {}
    )
    gate = (
    ...
    method_errors = analysis_artifacts.student_method_errors(
        current_solution, method_profile
    ) if current_solution else []
    artifact_dir = output_dir / "artifacts" / entry_id
    artifact_dir.mkdir(parents=True, exist_ok=True)
    if current_solution:
        (artifact_dir / "w3-recommended.md").write_text(
            current_solution, encoding="utf-8"
        )
    if report.get("blueprint"):
        (artifact_dir / "w3-blueprint.json").write_text(
            json.dumps(
                report["blueprint"], ensure_ascii=False, indent=2
            ) + "\n",
            encoding="utf-8",
        )
    if render.get("student_solution_md"):
        (artifact_dir / "student-solution.md").write_text(
            str(render["student_solution_md"]), encoding="utf-8"
        )
    if render.get("teacher_solution_md"):
        (artifact_dir / "teacher-solution.md").write_text(
            str(render["teacher_solution_md"]), encoding="utf-8"
        )
    ...
    ] if isinstance(telemetry, list) else []
    ...
    semantic_audit = (
        evidence.get("semantic_audit", {})
        if isinstance(evidence.get("semantic_audit"), dict)
        else {}
    )
    ...
    summary = {
        ...
        "provider_call_count": sum(
            1 for item in telemetry
            if isinstance(item, dict)
            and item.get("provider") not in {"checkpoint", None, ""}
        ),
        "checkpoint_replay_count": sum(
            1 for item in telemetry
            if isinstance(item, dict) and item.get("provider") == "checkpoint"
        ),
        ...
        "w3_solution_digest": text_digest(current_solution)
        if current_solution else "",
        "matches_prior_teacher_reviewed_w3": bool(
            prior_solution and current_solution == prior_solution
        ),
        "method_profile": method_profile,
        "method_gate_status": (
            "not-run"
        ...
        "stage_interface_status": evidence.get(
            "stage_interface_report", {}
        ).get("status", "missing"),
        "claim_aggregation_status": aggregation.get("status", "missing"),
        "claim_count": evidence.get("metrics", {}).get("claim_count", 0),
        "certificate_count": evidence.get("metrics", {}).get(
            "certificate_count", 0
        ),
        "unresolved_claim_count": evidence.get("metrics", {}).get(
        ...
        "student_character_count": len(
            str(render.get("student_solution_md", ""))
        ),
        "student_section_count": str(
            render.get("student_solution_md", "")
        ).count("## "),
