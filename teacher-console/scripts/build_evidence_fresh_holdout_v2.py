...
OVERLAY_PATH = "teacher-console/tests/fixtures/evidence-agent/fresh-holdout-v2-evidence-overlay.json"
PRIOR_DATASETS = (
...
def _evidence_id(source_id: str) -> str:
    digest = hashlib.sha256(f"fresh-holdout-v2:{source_id}".encode()).hexdigest()[:24]
    return f"EU-{digest}"
...
def units() -> list[dict[str, Any]]:
    return [
        ...
        evidence_unit(
            ...
            text=("理想变压器满足 U1/U2=N1/N2，且输入功率等于输出功率，所以 I1/I2=N2/N1；电压比与电流比互为倒数。"),
...
def cases(unit_by_section: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return [
        proposed_case(
            "fresh2-momentum-collision-valid",
            momentum,
            problem="两小车在水平轨道上短时碰撞，取两车为系统，碰撞期间外力冲量相对内力冲量可忽略。",
            purpose="applicability_check",
            question="该碰撞阶段采用系统动量守恒需要核对哪些条件？",
            facets=["系统边界", "外力冲量", "总动量守恒"],
            conflicts=["系统边界中途改变"],
            sufficient=True,
            rationale="系统固定且碰撞期间外力冲量可忽略，满足动量守恒判据。",
        ),
        proposed_case(
            "fresh2-momentum-explosion-valid",
            momentum,
            problem="静止物体在光滑水平面上爆炸成两部分，研究时间只取爆炸极短过程且系统包含全部碎片。",
            purpose="method_candidate",
            question="爆炸极短过程中为什么可以使用总动量守恒？",
            facets=["爆炸", "研究时间", "合外力冲量可忽略"],
            conflicts=["把机械能守恒当作动量守恒前提"],
            sufficient=True,
            rationale="全部碎片属于固定系统，短过程外力冲量可忽略，机械能是否守恒不是必要条件。",
        ),
        proposed_case(
            "fresh2-momentum-boundary-conflict",
            momentum,
            problem="解答在碰撞前把两车作为系统，碰撞后却只保留其中一车，仍直接声称前后系统总动量守恒。",
            purpose="exception_check",
            question="研究过程中改变系统边界后还能否直接比较所谓系统总动量？",
            facets=["系统边界", "研究时间", "外力冲量"],
            conflicts=["系统边界中途改变"],
            sufficient=False,
            rationale="前后研究对象不同，证据的固定系统前提被破坏。",
        ),
        proposed_case(
            "fresh2-lenz-field-increase-valid",
            lenz,
            problem="闭合线圈面积和方向不变，垂直纸面向里的匀强磁场正在增强，需要判断感应电流产生的磁场方向。",
            purpose="method_candidate",
            question="应用楞次定律时应怎样从原磁通的方向和增减判断感应磁场？",
            facets=["原磁通方向", "磁通量增加", "阻碍磁通变化"],
            conflicts=["把阻碍磁通量变化说成阻碍原磁场"],
            sufficient=True,
            rationale="闭合回路中磁通增加，证据给出从磁通变化到感应磁场的完整判断顺序。",
        ),
        proposed_case(
            "fresh2-lenz-area-decrease-valid",
            lenz,
            problem="闭合线圈处在方向不变的匀强磁场中，线圈有效面积逐渐减小，需要判断感应电流方向。",
            purpose="applicability_check",
            question="磁场本身不变但有效面积减小时，楞次定律是否仍依据磁通量变化？",
            facets=["闭合导体回路", "面积变化", "感应电流方向"],
            conflicts=["只看磁场方向而不看面积或夹角变化"],
            sufficient=True,
            rationale="有效面积变化导致磁通变化，证据明确要求同时检查面积和夹角。",
        ),
        proposed_case(
            "fresh2-lenz-open-loop-conflict",
            lenz,
            problem="一段导线与电阻之间存在断口，磁通量虽在变化，解答却画出了持续的闭合感应电流。",
            purpose="exception_check",
            question="回路存在断口时能否依据楞次定律直接声称存在持续感应电流？",
            facets=["闭合导体回路", "磁通量变化", "感应电流方向"],
            conflicts=["回路不闭合却声称有持续感应电流"],
            sufficient=False,
            rationale="磁通变化可产生感应电动势，但断路不满足持续闭合电流的适用条件。",
        ),
        proposed_case(
            "fresh2-transformer-voltage-valid",
            transformer,
            problem="理想变压器原、副线圈匝数分别为 N1、N2，原线圈接正弦交流电压 U1，需要求副线圈电压。",
            purpose="method_candidate",
            question="理想变压器的电压比怎样由匝数比确定？",
            facets=["理想变压器", "匝数比", "电压比"],
            conflicts=["直流稳态接入变压器仍套匝数比"],
            sufficient=True,
            rationale="明确为理想变压器和交流输入，可直接使用电压匝数比。",
        ),
        proposed_case(
            "fresh2-transformer-current-valid",
            transformer,
            problem="理想变压器稳定工作，已知原副线圈匝数比和副线圈负载电流，需要求原线圈电流。",
            purpose="verification_support",
            question="怎样利用功率守恒确认电流比与匝数比方向相反？",
            facets=["输入输出功率相等", "电流比", "匝数比互为倒数"],
            conflicts=["把电流比写成匝数比同向"],
            sufficient=True,
            rationale="理想条件下功率守恒，证据明确给出电流比与匝数比的倒数关系。",
        ),
        proposed_case(
            "fresh2-transformer-loss-conflict",
            transformer,
            problem="变压器效率明确只有 80%，解答仍令输入功率等于输出功率并直接套用理想电流比。",
            purpose="exception_check",
            question="存在明确能量损耗时还能否直接使用理想变压器的功率与电流比？",
            facets=["理想变压器", "功率守恒", "电流比"],
            conflicts=["非理想变压器仍直接令输入功率等于输出功率"],
            sufficient=False,
            rationale="效率小于 100% 直接破坏理想功率守恒前提。",
        ),
        proposed_case(
            "fresh2-interference-coherent-valid",
            interference,
            problem="两列频率相同、相位差恒定的简谐波在同一区域持续叠加，需要判断是否形成稳定干涉图样。",
            purpose="applicability_check",
            question="稳定干涉需要满足哪些相干条件？",
            facets=["频率相同", "相位差恒定", "观察区域叠加"],
            conflicts=["只因两列波相遇就判定稳定干涉"],
            sufficient=True,
            rationale="频率、相位差和空间叠加三个条件均明确满足。",
        ),
        proposed_case(
            "fresh2-interference-amplitude-valid",
            interference,
            problem="两列相干波频率相同且相位差恒定，但振幅不同，在观察区域叠加。",
            purpose="boundary_check",
            question="振幅不同是否会破坏稳定干涉的相干条件？",
            facets=["稳定干涉图样", "频率相同", "相位差恒定"],
            conflicts=["把振幅相同当作相干的充分条件"],
            sufficient=True,
            rationale="振幅相同不是稳定干涉的必要条件，关键是频率相同和相位差恒定。",
        ),
        proposed_case(
            "fresh2-interference-frequency-conflict",
            interference,
            problem="两列波频率略有不同，解答仅因它们在空间相遇就声称各加强点位置长期固定。",
            purpose="false_friend_check",
            question="频率不同的两列波能否形成条纹位置稳定的干涉图样？",
            facets=["频率相同", "相位差恒定", "稳定干涉图样"],
            conflicts=[],
            sufficient=True,
            rationale="证据明确给出稳定干涉要求频率相同、相位差恒定，因此可以直接纠正“频率不同仍有稳定图样”的错误结论。",
            diagnostic_targets=["频率不同仍声称条纹位置稳定"],
        ),
        proposed_case(
            "fresh2-relative-interior-valid",
            relative,
            problem="甲乙在同一惯性系中均做匀速直线运动，计算得到某时刻二者相对位置与相对速度垂直，且该时刻位于题设时间区间内。",
            purpose="method_candidate",
            question="匀速相对运动中如何判定区间内部的最近时刻？",
            facets=["相对位置", "相对速度", "垂直", "时间区间"],
            conflicts=["存在加速度仍把相对速度当常量"],
            sufficient=True,
            rationale="匀速和区间内部条件均满足，可用垂直条件判最近时刻。",
        ),
        proposed_case(
            "fresh2-relative-endpoint-valid",
            relative,
            problem="两物体均做匀速直线运动，但由相对位置与相对速度垂直求出的时刻早于题设观察区间。",
            purpose="boundary_check",
            question="形式上求得的最近时刻不在观察区间时应怎样处理？",
            facets=["研究时间区间", "垂直时刻", "区间端点"],
            conflicts=["求得的垂直时刻不在区间内却不检查端点"],
            sufficient=True,
            rationale="证据明确要求内部解越界时比较区间端点。",
        ),
        proposed_case(
            "fresh2-relative-acceleration-conflict",
            relative,
            problem="乙物体正在做匀加速运动，解答仍把初始相对速度当作全程常量并套用相对位置垂直条件。",
            purpose="exception_check",
            question="存在加速度时能否直接使用恒定相对速度的最近距离结论？",
            facets=["匀速直线运动", "相对速度", "最近距离"],
            conflicts=["存在加速度仍把相对速度当常量"],
            sufficient=False,
            rationale="相对速度随时间变化，固定速度条件不成立。",
        ),
        proposed_case(
            "fresh2-satellite-speed-valid",
            satellite,
            problem="两颗卫星绕同一行星做半径不同的圆周运动，需要比较轨道线速度。",
            purpose="method_candidate",
            question="同一中心天体圆轨道的线速度怎样随轨道半径变化？",
            facets=["同一中心天体", "圆轨道", "线速度", "轨道半径"],
            conflicts=["比较不同中心天体时忽略中心天体质量"],
            sufficient=True,
            rationale="同一中心天体且均为圆轨道，可使用 v 与 r 的比例关系。",
        ),
        proposed_case(
            "fresh2-satellite-period-valid",
            satellite,
            problem="同步比较绕同一中心天体的两个圆轨道，外轨道半径更大，需要判断周期大小。",
            purpose="verification_support",
            question="圆轨道周期随轨道半径怎样变化？",
            facets=["周期", "轨道半径", "同一中心天体"],
            conflicts=["把轨道高度直接当作轨道半径"],
            sufficient=True,
            rationale="满足同一中心天体和圆轨道条件，半径更大周期更大。",
        ),
        proposed_case(
            "fresh2-satellite-ellipse-conflict",
            satellite,
            problem="卫星沿明显的椭圆轨道运动，解答把当前到中心天体的距离直接代入圆轨道 v=√(GM/r) 求瞬时速度。",
            purpose="exception_check",
            question="椭圆轨道某点的瞬时速度能否直接使用圆轨道速度公式？",
            facets=["圆轨道", "瞬时速度", "轨道半径"],
            conflicts=["把椭圆轨道瞬时速度直接代入圆轨道比例"],
            sufficient=False,
            rationale="当前轨道不是圆轨道，圆轨道向心条件不能直接迁移。",
        ),
        proposed_case(
            "fresh2-photo-frequency-valid",
            photoelectric,
            problem="同一种金属受高于截止频率的单色光照射，现提高入射光频率并保持其他条件不变。",
            purpose="method_candidate",
            question="提高频率会怎样影响光电子最大初动能？",
            facets=["最大初动能", "频率", "逸出功"],
            conflicts=["只提高光强就声称最大初动能增大"],
            sufficient=True,
            rationale="同一种金属的逸出功固定，爱因斯坦方程直接给出频率对最大初动能的影响。",
        ),
        proposed_case(
            "fresh2-photo-intensity-valid",
            photoelectric,
            problem="高于截止频率的单色光照射同一种金属，只提高光强，需要区分光电子最大初动能和单位时间逸出数。",
            purpose="verification_support",
            question="提高光强主要改变光电效应中的哪个量？",
            facets=["光强", "光电子数", "最大初动能"],
            conflicts=["只提高光强就声称最大初动能增大"],
            sufficient=True,
            rationale="光频率不变时单个光子能量不变，光强主要改变光电子数。",
        ),
        proposed_case(
            "fresh2-photo-below-threshold-conflict",
            photoelectric,
            problem="入射单色光频率低于该金属截止频率，解答声称只要把光强调得足够大就能逸出光电子。",
            purpose="false_friend_check",
            question="低于截止频率时增大光强能否触发光电效应？",
            facets=["截止频率", "光强", "光电子"],
            conflicts=[],
            sufficient=True,
            rationale="证据明确说明低于截止频率时提高光强也不能发生光电效应，因此能够直接纠正当前错误结论。",
            diagnostic_targets=["低于截止频率时靠增大光强产生光电子"],
        ),
    ]
...
def build() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    overlap = prior_evidence_ids() & {item["evidence_id"] for item in overlay_units}
    if overlap:
    ...
    overlay["overlay_fingerprint"] = evidence_contract.stable_fingerprint("evidence-overlay-v1", overlay_units)
