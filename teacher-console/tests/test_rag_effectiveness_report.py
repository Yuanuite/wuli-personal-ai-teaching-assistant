...
SPEC = importlib.util.spec_from_file_location("rag_effectiveness_report", ROOT / "teacher-console" / "scripts" / "rag_effectiveness_report.py")
...
class RagEffectivenessReportTest(unittest.TestCase):
    ...
    def test_observational_report_groups_rag_and_legacy_outcomes(self):
        self.append(
            "answer.revise", "agent", "agent-result", "completed",
        ...
        self.append(
            "answer.revise", "agent", "agent-result", "failed",
        ...
        for job_id, context, status in (
            ...
            kb.write_json(self.jobs / f"{job_id}.json", {
                "schema_version": 1,
                "id": job_id,
                "kind": "answer.revise",
                "entry_id": "entry-1",
