...
class CorrectnessAblationTest(unittest.TestCase):
    def test_loop_ablation_keeps_conditions_fixed_and_never_promotes_faults(self):
        report = correctness_ablation.run_ablation(generated_at="2026-07-29T00:00:00+00:00")
        ...
        self.assertEqual(report["metrics"]["association_truth_promotion_count"], 0)
    ...
    def test_fault_paths_gain_bounded_challenges_and_fuse(self):
        ...
        fault_cases = [item for item in report["cases"] if item["injected_verdict"] != "pass"]
        ...
        for item in fault_cases:
            with self.subTest(scenario=item["scenario"]):
                ...
                self.assertNotEqual(item["on"]["aggregation_status"], "VERIFIED")

    def test_report_is_replayable_under_fixed_timestamp(self):
        first = correctness_ablation.run_ablation(generated_at="2026-07-29T00:00:00+00:00")
        second = correctness_ablation.run_ablation(generated_at="2026-07-29T00:00:00+00:00")
