...
class CorrectnessReplayTest(unittest.TestCase):
    def setUp(self):
        ...
        write_json(entry / "w3-shadow-report.json", {
            "status": "completed",
            "report": {"blueprint": blueprint, "solver_a": solver},
        })
        write_json(self.experiment / "manifest.json", {
            "schema_version": 2,
        ...
        write_json(self.experiment / "truth-lock.json", {
            "case_count": 1,
            "target_count": 1,
            "cases": [{"entry_id": self.entry_id, "target_count": 1}],
        })
        write_json(self.experiment / "result.json", {
            "gates": {"production_eligible": False}
        })
        write_json(self.experiment / "paired-result.json", {
    ...
    def test_legacy_replay_projects_but_never_invents_certificates(self):
        ...
        self.assertEqual(
            report["metrics"]["provisional_without_certificate_count"], 1
        )
    ...
    def test_changed_reference_digest_is_reported_not_silently_scored(self):
        answer = (
            self.library / "entries" / self.entry_id / "student-solution.md"
        )
        ...
        self.assertEqual(
            report["metrics"]["reference_digest_mismatch_count"], 1
        )
