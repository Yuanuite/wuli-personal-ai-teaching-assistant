...
class W3RContractTest(unittest.TestCase):
    ...
    def test_unverified_package_never_reaches_renderer(self):
        ...
        result = w3r_contract.build_w3r_brief(source["problem"], source["blueprint"], source["proof_package"])
        self.assertEqual(result["status"], "needs_render_material")
        self.assertEqual(result["violations"][0]["code"], "proof-package-not-verified")
    ...
    def test_olympiad_profile_is_explicit_in_render_brief(self):
        ...
        self.assertEqual(result["brief"]["method_scope"]["level"], "olympiad_official")
        self.assertNotIn(
    ...
    def test_dependency_cycle_fails_preflight(self):
        ...
        built = w3r_contract.build_w3r_brief(source["problem"], source["blueprint"], source["proof_package"])
        brief = copy.deepcopy(built["brief"])
        brief["proof_skeleton"][0]["depends_on"] = [brief["proof_skeleton"][-1]["step_id"]]
    ...
    def test_unresolved_obligation_returns_needs_material(self):
        source = fixture()
        source["proof_package"]["unresolved_obligations"] = [{"obligation_id": "v1"}]
        result = w3r_contract.build_w3r_brief(source["problem"], source["blueprint"], source["proof_package"])
        self.assertEqual(result["status"], "needs_render_material")
        self.assertTrue(any(item["code"] == "unresolved-verification-obligation" for item in result["violations"]))
    ...
    def test_proof_skeleton_over_student_step_budget_needs_material(self):
        ...
        result = w3r_contract.preflight_w3r_brief(brief)
        self.assertEqual(result["status"], "needs_render_material")
        self.assertTrue(any(item["code"] == "proof-mainline-too-long" for item in result["violations"]))
