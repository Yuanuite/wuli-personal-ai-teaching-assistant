...
项目最核心的价值不是“再生成一份答案”，而是：

> **把一道学生错题，转化为一条经过教师确认、能够用于课堂讲解、可以支持课后学习、并会持续沉淀复用的教学资产。**
...
2. **可信任**：教师复核门禁、版本摘要和确定性验证共同约束 AI；
3. **会生长**：错题、讲法、反馈与失败经验进入本地知识库，为下一次处理提供证据。
...
截至 2026 年 7 月 29 日，项目已完成可运行的高中物理 MVP：

- 已建立教师本地工作台和只读学生静态站；
- 已跑通“上传—OCR—题干复核—分层解析—答案复核—按需仿真—交付—公开预览”的完整流程；
- 本地样例知识库现有 23 个条目，均已 `delivered` 并进入本地学生站目录；
- 12 个条目包含结构化物理模型，可用于验证动态可视化流程；
- 30 条教师确认固定检索集上，本轮 Hit@5 为 1.0000、Recall@5 为 0.8292、MRR 为 0.8633；Recall@5 仍低于项目 85% 门槛，不能据此宣称答案质量已提升；
- 已具备 Markdown、PDF、离线 HTML 和学生包输出；
- 已实现公开题图裁剪、遮挡、教师确认和隐私白名单；
- Claim Evidence 与受控认知环仍是默认关闭的私有影子层，已实现 Claim DAG、确定性证书、隔离语义复算、依赖定向回跳和受控假设搜索；14 类故障注入全部检出且错误晋升为 0。独立于该影子层，W3 自适应复杂题路由已通过新鲜 holdout（W2/W3 均 15/15）、首批灰度（9/9）和官方竞赛补充灰度（8/8），现默认把复杂题路由到 W3，低风险题和失败回退继续使用 W2；
- 385 项 Python 全量自动化测试通过，覆盖答案复核失效、Agent Gateway 隔离与降级、结构化解析、Claim Ledger、受控认知环、自适应路由与回滚、模型注册表/逐模型测试、后台作业、反馈事件链、派生检索一致性、文件夹同步、可视化门禁、公开发布与隐私边界；另有 4 条隔离 E2E 场景实际操作浏览器与本地 API。
