...
**现象**：编辑答案后，可视化审批状态从 `passed` 变为 `stale`。

**原因**：设计行为——答案与可视化可能已不对齐，需重新复核。与“可视化生成后答案审批自动失效”对称。

**处置**：重新预览可视化，确认与修改后的答案一致后重新批准。

### 难度量表自动保存

**现象**：编辑难度量表字段后，页面右下角短暂显示“已保存”。

**原因**：设计行为——难度量表字段（维度评分、核心判断、摘要、校准依据）在编辑后 800ms 自动保存，无需手动点击保存按钮。

**处置**：无需操作，自动保存即可。如需手动触发，仍可点击“保存难度量表”按钮。
