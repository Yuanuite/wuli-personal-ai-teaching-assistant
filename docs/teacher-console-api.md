...
该 API 是 `teacher-console/` 页面与本地生命周期脚本之间的内部接口，不是面向公网的开放 API，也不供 `student-site/` 调用。服务默认只监听 `127.0.0.1`。
...
缺少该请求头时返回 `403`。流程门禁未满足时，操作通常返回 `409`，JSON 中的 `status` 为 `blocked` 并附带 `errors`；页面和外部调用方都不得把它当作成功。

## 只读接口

| 方法与路径 | 作用 |
|---|---|
| `GET /api/health` | 服务状态、项目位置和页面 Agent 可用性 |
...
文件接口会解析并检查规范路径，禁止跳出各自的条目、草稿、公开站或交付目录。可视化接口只允许两个固定文件名。
...
| `analyze` | 可选 `instruction` | 调用或排队当前 Agent 生成分层解析；不会自动生成交互仿真 |
| `save-answer` | `layer`、`markdown`、可选 `base_digest` | 保存学生版或教师版 Markdown，并撤销旧答案批准 |
| `approve-answer` | `reviewer`、`note` | 批准当前题干、答案、模型和引用图片的联合摘要 |
| `request-revision` | 修改意见及页面提供的版本摘要 | 让 Agent 在当前条目范围内返修答案和解释图 |
| `build-visualization` | 可选 `message`、`runtime_check` | 无模型时请求 Agent 创建模型；有模型时构建预审仿真 |
| `approve-visualization` | `reviewer`、`note` | 批准当前模型、HTML/ZIP 和运行证据摘要 |
| `visualization-chat` | `message`、可选 `base_digest` | 请求创建或修复当前题目的交互仿真 |
