...
| Agent provider、模型选择、API Key、后台任务 | `docs/agent-gateway.md` 的兼容矩阵、`docs/failure-intelligence.md` | `teacher-console/agent_gateway.py`、`teacher-console/model_registry.py`、provider adapter、对应测试 | Gateway、模型注册、失败排障、静态契约测试；改 `analysis.generate` 契约还要同步单元/E2E 两个 fake adapter 并跑 3 条 E2E |
