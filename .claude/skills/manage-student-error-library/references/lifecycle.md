...
5. Have the teacher review both answer layers and their referenced images, then run `approve-answer`. If revision is requested, return to analysis; the workbench submits a persistent asynchronous Agent Gateway job and polls it while preserving unsaved teacher text. The provider edits only a disposable candidate, and allowlisted files reach canonical storage only after validation. Even then, the teacher must approve the result again. Changing Markdown, the shared model, or a referenced SVG/PNG invalidates the prior approval.
...
9. Open the reported PDF and simulator screenshot when available. Report only the artifacts listed in `delivery-manifest.json`.

The Agent Gateway job state (`queued → running → completed|failed`) is operational state, not a lifecycle state. `completed` means a candidate passed mechanical/domain validation and was promoted; it does not mean source, answer, visualization, delivery, or publication approval. A teacher-selected `routing_tier` (`auto|economy|expert`) may change provider model and least-disclosure context, but never these gates. Record the requested tier, actual model/tier, honest fallback notice, and provider usage when available. On provider failure or server restart, leave canonical content and lifecycle gates unchanged and let the teacher resubmit or edit manually; escalation uses a fresh job, never a second model layered onto a changed candidate.
