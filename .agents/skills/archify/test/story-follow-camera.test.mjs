...
test('Story Follow frames the exact previous, current, and next authored stops through the shared camera', () => {
  ...
  assert.match(template, /primaryId: step\.nodeId/);
...
test('Story Follow keeps the current stop visible on contained narrow diagrams', () => {
  assert.match(template, /if \(options\.primaryId && !container\.hasAttribute\('data-wide-diagram'\)\) return frameDesktop\(ids, options\)/);
  assert.match(template, /var primaryBoxes = options\.primaryId \? boxesFor\(\[options\.primaryId\], false\) : \[\]/);
  assert.match(template, /var storyVisibleEnough = primaryBounds && visibleHeight >= primaryBounds\.height \+ 32/);
  assert.match(template, /Math\.min\(padding, Math\.max\(16, \(visibleHeight - primaryBounds\.height\) \/ 2\)\)/);
  assert.match(template, /allowSemanticOverflow = !!primaryBounds/);
  assert.match(template, /if \(allowSemanticOverflow && state\.mode === 'semantic'\)/);
  assert.match(template, /function interruptCamera\(reason\)[\s\S]*?allowSemanticOverflow = false/);
  assert.match(template, /if \(primaryLeft < left\) target\.x \+= left - primaryLeft/);
  assert.match(template, /else if \(primaryRight > right\) target\.x -= primaryRight - right/);
  assert.match(template, /if \(primaryTop < top\) target\.y \+= top - primaryTop/);
  assert.match(template, /else if \(primaryBottom > bottom\) target\.y -= primaryBottom - bottom/);
});
