...
$$
- （3）按题干“某时刻”的字面条件，完整解集为
$$
z_0=l,\qquad x_0\in\mathbb R,
$$
或
$$
(x_0,z_0)=\left[-\frac{2(k+1)mv_0}{(k+2)eB},\frac{k^2l}{(k+2)^2}\right].
$$
第一类中，当 $x_0=0$ 时发射延迟可任取正值；当 $x_0\ne0$ 时，发射延迟必须为整数个回旋周期。第二类中 B 先进入电场 II，再返回分界面并进入磁场。

## 一眼识别

- 题型识别：电场中的类平抛运动、磁场中的螺旋运动与相对运动综合题。
- 最短主线：先由 A 恰好从 $P$ 到达 $O$ 求电场强度，再分解螺旋运动；最后把“两电子距离不变”分为“共圆心”和“同相位”两种条件。
...
两电子的横向相对位置可看成“固定的圆心差”与“以相同角速度转动的矢量差”之和。其长度始终不变有两种方式：

1. 两电子的回旋圆心相同；
2. 两电子的回旋半径和相位相同，此时转动矢量差为零。
...
A 的回旋圆心为 $(-r,0)$。采用第一种方式时，故
$$
z_f=0,
$$
且
$$
x_0+\frac{mv_{zf}}{eB}=-r. \qquad \text{（1）}
$$
...
此时 B 与 A 的入场速度相同、回旋半径相同。若采用共圆心条件，代入式（1）得
$$
x_0=0.
$$

但题目只说 B 在 A 发射后的“某时刻”出发，没有限制延迟。设发射延迟为 $\Delta t$，若
$$
\Delta t=nT,\qquad n=1,2,3,\ldots,
$$
则 B 进入磁场时，A 恰好转过整数周，两电子的回旋半径与相位相同。此时即使圆心横向相差 $x_0$，横向位置差仍恒为 $x_0$，所以任意 $x_0$ 都成立。

因此这一情况的完整结果是
$$
\boxed{z_0=l,\qquad x_0\in\mathbb R}.
$$
其中 $x_0=0$ 时不要求 $\Delta t=nT$；$x_0\ne0$ 时必须有 $\Delta t=nT$。
...
电子 B 的电场路径只有“始终在电场 I”和“进入电场 II 后返回”两类；进入磁场后的恒距条件又只有“共圆心”或“同半径同相位”两类。跨界分支的回旋半径为 $kr/(k+2)\ne r$，不能产生同相位补充分支，故上述枚举完备。

## 易错点

- **错误表现**：认为电子在电场 I 中沿 $+z$ 加速；**纠正策略**：电子带负电，受力方向与电场方向相反。
- **错误表现**：把磁场中的轨迹直接看成平面圆；**纠正策略**：先将速度分为平行磁场和垂直磁场两个分量，合运动是螺旋运动。
- **错误表现**：把“共圆心”当作唯一恒距条件；**纠正策略**：还要检查“同半径、同相位”使转动矢量差为零的退化情形。

## 30 秒自测

为什么当 B 晚发射整数个回旋周期且 $z_0=l$ 时，任意 $x_0$ 都能使两电子进入磁场后距离不变？
