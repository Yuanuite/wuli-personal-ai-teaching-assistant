...
- **错误表现**：把 $B_0$ 当作产生感应电流的磁场；**纠正策略**：$B_0$ 负责安培力（左手定则），可变磁场负责产生感应电流（楞次定律），两者职责不同，不要混用。
- **错误表现**：混淆左手定则和右手定则的使用场景；**纠正策略**：判断受力方向用**左手**（$F=IL\times B$），判断感应电流/感应磁场方向用**右手**（螺旋定则 + 楞次定律）。
- **错误表现**：漏掉匝数 $N$；**纠正策略**：感应电动势和安培力两个环节都含 $N$，应分别写出。
